...
if (isset($_SESSION['email'])) {
    ...
    if (count($errors) == 0) {
        ...
        if ($result == null) {
            ...
            $errors["password"] = "Email y/o constraseña incorrectos";
        } else {
            $auth->login($user->getEmail());
            $auth->createSession($result);
            if (isset($_POST['recordar'])) {
                $auth->remember($_POST['password']);
            }
            rediret('index.php');
        }
    }
            ...
            <div class="login_section">
                <div class="form_content">
                    <form class="form_signup" action="" method="post">
                        ...
                        <div class="pass">
                            <input class="input_change" type="password" name="password" value="<?= (isset($_COOKIE['password'])) ? $_COOKIE['password'] : '' ?>" required>
